    ...
    $.fn.Transformer = function() {
      ...
      if (arguments.length === 0 || ((options = arguments[0]) && typeof options === 'object')) {
        ...
        this.each(function(_index, _value) {
          ...
          if (!exists) {
            return existingTransformer.push(new Transformer(_value, options));
          }
        });
      } else if (arguments.length > 0) {
        args = arguments;
        this.each(function(_index, _value) {
          var et, transformer, _i, _len;
          for (_i = 0, _len = existingTransformer.length; _i < _len; _i++) {
            et = existingTransformer[_i];
          ...
          if (transformer != null) {
            return transformer[args[0]].apply(transformer, Array.prototype.slice.call(args, 1, args.length));
          }
        });
      }
      return this;
    };
    return $(document).on('DOMNodeRemoved', function(_e) {
      var i, index, item, _i, _len;
      index = -1;
      for (i = _i = 0, _len = existingTransformer.length; _i < _len; i = ++_i) {
        item = existingTransformer[i];
      ...
      if (~i) {
        return existingTransformer.splice(i, 1);
      }
    });
  })($);
